...
for ((n=StartCycleNum; n<=nCycle; n++)) ; do
    ...
    sedArgs=`cat <<EOF 
     s!#set_solar_const_nml_SolarConst#!${SolarConst}.0!g;
     ...
     s!#gtool_historyauto_nml_IntValue#!100.0!g; 
EOF
    ` 
    ...
    sedArgs=`cat << EOF
     ...
     s!#Exp_APECoupleClimate_nml_SfcBCDataDir#!${ocn_wdir}/cycle$((n-1))-standalone/!g;
     s!#Exp_APECoupleClimate_nml_MeanInitTime#!$((250*365))!g;
     s!#Exp_APECoupleClimate_nml_MeanEndTime#!${standaloneTimeIntrvPerCycle}.0!g;
EOF
    `
	...
	cp    ${EXPDIR}/DCCM.conf ${atmDirPath}
	cp    ${EXPDIR}/DCCM.conf ${ocnDirPath}
