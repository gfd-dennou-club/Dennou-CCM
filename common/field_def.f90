...
subroutine set_field_def(component_name, grid_name, &
  ...
  integer, intent(in), optional :: lnx_field(div_x), lny_field(div_y)
  integer :: i
  ...
  if(present(lnx_field)) then
     current_field%lnx_field(:) = lnx_field
  else
     call cal_local_size(current_field%GNX, current_field%DIVX, current_field%lnx_field)
  end if

  if(present(lny_field)) then
     current_field%lny_field(:) = lny_field     
  else
